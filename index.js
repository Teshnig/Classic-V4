...
say('ZIM-BOT\nV4', {
  ...
  say(`CLASSIC BOT By @${author.name || author}`, {
    font: 'console',
    align: 'center',
    gradient: ['red', 'green']
  })
Drips.hr();
console.log(color(''), color('THANKS FOR CHOOSING CLASSIC BOT BOT', 'green'))
console.log(color(''), color('SCRIPT BY SAM', 'red'))
console.log(color( ''), color('https://wa.me/+258869103969','cyan'))
Drips.hr();
Drips.banner(`SAM CLASSIC OWNER`)
Drips.ok('WELCOME TO CLASSIC BOT')
...
async function startZimBotInc() {
	...
	const { state, saveCreds } = await useMultiFileAuthState('Classic_Session')
    const ZimBotInc = ZimBotIncConnect({
        printQRInTerminal: true,
         syncFullHistory: true,
         logger: logg({ level: 'silent' }),
         auth: state,
         browser: ["CLASSIC-BOT", "Safari", "3.0"],
    ...
    ZimBotInc.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./Classicbot")(ZimBotInc, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
 ...
 ZimBotInc.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
              ...
              if ( !member.startsWith('263') || !member.startsWith('254'))  {// put your country code for localonly 
              ZimBotInc.sendMessage(anu.id, { text: `Sorry @${member.split`@`[0]}, this group is only for KENYAN citizens and you will be removed automatically.`})
...
const driphunny = fs.readFileSync('./Classicbot/welcome.mp3')
...
//by drips
function scheduleGc() {// garbage collector by Sam
